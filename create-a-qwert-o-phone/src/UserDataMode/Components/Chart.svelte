...
<div class="chart">
	...
	<Transposition
		bind:transpositionData={chartData.TranspositionEventCodes}
		listString={ListString}
		bind:listIndex={chartData.ChartID}
	/>
		...
		<div>
			...
			<div class="pad">
				{#each $QOPUserData.ChartList[chartData.ChartID].PadSet as pad, padIndex}
					<Pad padData={pad}>
						<button on:click={() => handleRemovePad(chartData.ChartID, padIndex)}
							>Remove Pad {padIndex}</button
						>
					</Pad>
				{/each}
			</div>
			...
			<div class="combo">
				{#each $QOPUserData.ChartList[chartData.ChartID].ComboSet as combo, comboIndex}
					<Combo comboData={combo}>
						<button on:click={() => handleRemoveCombo(chartData.ChartID, comboIndex)}
							>Remove Combo {comboIndex}</button
						>
					</Combo>
				{/each}
			</div>
